...
RUN cd /build/mapbox-gl-native && BUILDTYPE=Debug && make render
...
COPY --from=build /build/mapbox-gl-native/build/linux-x86_64/Debug/mbgl-render /usr/local/bin/mbgl-render
COPY --from=build /build/mapbox-gl-native/common/ca-bundle.crt /etc/ssl/certs/ca-bundle.crt
COPY ./docker-entrypoint.sh /usr/local/bin/
ENTRYPOINT ["/usr/local/bin/docker-entrypoint.sh"]
#ENTRYPOINT ["xvfb-run", "-a", "-e", "/dev/stdout", "--server-args=-screen 0 1024x768x24", "/usr/local/bin/mbgl-render"]
